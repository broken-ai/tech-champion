...
typedef struct Layer {
    ...
    float xSpeed; // How much the layer moves horizontally compared to camera (1 is normal scrolling with player movement, fixed for layer 4)
    float ySpeed; // How much the layer moves vertically compared to camera (1 is normal scrolling with player movement, fixed for layer 4)
...
```
Map *_map_ = new Map();
if (!_map_->load(gameBaseFolder + "content/" "test.tcm")) {
...
public:
    ...
    ~Map();
    bool load(std::string fileName, const std::string &inPath);

    /**
        Updates and draws a piece of layers 0-4 based on camera position and layer position
    */
    void drawBackground(sf::RenderWindow* inWindow, const int &inCamX, const int &inCamY, Config* inConfig);

    /**
        Updates and draws a piece of layers 5-7 based on camera position and layer position
    */
    void drawForeground(sf::RenderWindow* inWindow, const int &inCamX, const int &inCamY, Config* inConfig);
