...
void Map::drawBackground(sf::RenderWindow* inWindow, const int &inCamX, const int &inCamY) {
    int startX;
    int startY;
    int endX;
    int endY;
    ...
    for (int layerIndex = 0; layerIndex < 5; layerIndex++) {
        ...
        endX = (int)((inCamX + g_config.WINDOW_WIDTH + layerOffsetX) / 32);
        if (endX > this->layers[layerIndex].width - 1) endX = this->layers[layerIndex].width - 1;
        endY = (int)((inCamY + g_config.WINDOW_HEIGHT + layerOffsetY) / 32);
...
void Map::drawForeground(sf::RenderWindow* inWindow, const int &inCamX, const int &inCamY) {
    int startX;
    int startY;
    int endX;
    int endY;
...
bool Map::load(const std::string &fileName) {  /// Bigger than before, but safer
